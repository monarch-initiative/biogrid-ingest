"""Command line interface for biogrid-ingest."""
import logging
...
def callback(version: bool = typer.Option(False, "--version", is_eager=True),
    ...
    """biogrid-ingest CLI."""
    if version:
        from biogrid_ingest import __version__
        typer.echo(f"biogrid-ingest version: {__version__}")
        raise typer.Exit()


@app.command()
def download(force: bool = typer.Option(False, help="Force download of data, even if it exists")):
    """Download data for biogrid-ingest."""
    typer.echo("Downloading data for biogrid-ingest...")
...
def transform(
    ...
    """Run the Koza transform for biogrid-ingest."""
    typer.echo("Transforming data for biogrid-ingest...")
